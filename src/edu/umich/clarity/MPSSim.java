...
//			 System.out.println("START ready: "+issuingQueries.get(kernel.getQuery_type()).getReady_time()
//			 +"; elapse: "+elapse_time+", kernel elapse: "+kernel_elapse_time+", overlapping: "+overlapping_time+", select: "+chosen_query);
...
//					if(kernel.getQuery_type()==0 && kernel.getOccupancy() !=0)
//					 System.out.println("-----ready: "+issuingQueries.get(kernel.getQuery_type()).getReady_time()
...
*/
							...
							kernel.setReal_duration(kernel.getDuration()
									* batches / org_batches
									* microDelays.get(kernel.getQuery_type()));
//							System.out.println("before-------start: "+kernel.getStart_time()+" , end: "+kernel.getDuration()+" org_batches: "+org_batches);
...
//								+ " --> " + mc.getK().getEnd_time() + ", current: " + current_time);
					...
					else if(Float.compare(mc.getK().getEnd_time(), kernel.getStart_time()+5.0f) > 0) {
						active_pageable ++;
					}
				}
				...
				//////////Delay memcpy time
///*
				if(active_pageable > 0 && Float.compare(kernel.getDuration(), 1.0f)>0) {
					issuingQueries.get(kernel.getQuery_type()).setOverhead(
							issuingQueries.get(kernel.getQuery_type()).getOverhead()
							+(0.15f+0.2f*randQuery.nextFloat())*active_pageable
							);
				
//					start_t = current_time + (0.1f+0.1f*randQuery.nextFloat())*active_pageable;
//					end_t = start_t + kernel.getDuration();
//					current.getK().setStart_time(start_t);
//					current.getK().setEnd_time(end_t); // update the end_time first, for sorting in the next step
//					if(current.getK().getEnd_time() > issuingQueries.get(current.getK().getQuery_type()).getReady_time())
//						issuingQueries.get(current.getK().getQuery_type())
...
//						System.out.println("start time updated from: "+current_time+"--------------->"+current.getK().getStart_time()
//								+"=====end time updated from: "+current_time + kernel.getDuration()+"--------------->"+current.getK().getEnd_time());
//					}
...
///*					
						if(Float.compare(kernel.getDuration(),0.005f) >0) {
//						System.out.println("org parallel is: "+parallel+",  pre parallel is: "+pre_parallel+", other side: "+(int)Math.ceil(background/3.0f));
						pre_parallel += (int)Math.ceil(background/3.0f);
//						pre_parallel += (int)(background/3.0f);
						parallel += (int)Math.ceil(background/3.0f);
						}
//*/						
//						System.out.println("AFTER org parallel is:--------"+parallel+"pre parallel is: "+pre_parallel);
						
						for (int j = i - 1; j < active_memcpy_htd.size(); j++) {
							float ll = 2.8f + randQuery.nextFloat() * 0.5f;
...
//							 if(active_memcpy_htd.get(j).getK().getQuery_type() == 0)
							if(Float.compare(start_t, 20000.0f)>0)
							 System.out.println("memcpy: "+j+" of "+active_memcpy_htd.size()
									 +"===========raw_slow: "+raw_slow
									 +"-->slow: "+slow
									 +",,,,,,,,,, segment length: " + (interval.get(i) - interval.get(i - 1))
									 +"--------append addition: "+raw_time*slow);
*/							
...
//											+ 0.02f
					...
					if(Float.compare(kernel.getDuration(),0.005f) >0) {
					for (int i = 0; i < active_memcpy_htd.size(); i++) {
						if(Float.compare(kernel.getDuration(),2.0f) >0 && 
								Float.compare(active_memcpy_htd.get(i).getExpected_left_time(), 3.0f) < 0)
							active_memcpy_htd.get(i).setExpected_left_time(0.4f * randQuery.nextFloat() + active_memcpy_htd.get(i).getExpected_left_time());
//						else 
//							active_memcpy_htd.get(i).setExpected_left_time(0.1f * randQuery.nextFloat() + active_memcpy_htd.get(i).getExpected_left_time());
						active_memcpy_htd.get(i).getK()
								.setEnd_time(start_t+ active_memcpy_htd.get(i).getExpected_left_time());
/*						
						if(start_t > 20000.0f)
						System.out.println("duration of "+ active_memcpy_htd.get(i).getK().getQuery_type()+" : "+
												active_memcpy_htd.get(i).getK().getExecution_order()+"---->updated to: "
								+ (active_memcpy_htd.get(i).getK()
										.getEnd_time() - active_memcpy_htd
										.get(i).getK().getStart_time())
								+", end at: "+active_memcpy_htd.get(i).getK()
								.getEnd_time());
*/						
//						System.out.println("add new-------------duration updated to: start at: "
//								+ start_t+", left: "+active_memcpy_htd.get(i).getExpected_left_time()+
//								", client: "+active_memcpy_htd.get(i).getK().getQuery_type());						
/*						
...
//							if(active_memcpy_htd.get(i).getK().getQuery_type()==0)
//								System.out.println(current.getK().getExecution_order()+": "
//											+active_memcpy_htd.get(i).getK().getExecution_order()
//											+": ready at: "+ active_memcpy_htd.get(i).getK().getEnd_time());
...
//							System.out.println("new is: "+active_memcpy_htd.get(i).getK().getQuery_type() + 
//									"end: "+active_memcpy_htd.get(i).getK().getEnd_time()
//									+ ", current time: "+current.getK().getEnd_time());
...
//					System.out.println("Add a pinned memory copy!!!!!"+active_memcpy_htd.size()+", duration is: "+current.getK().getDuration());
					...
					for (int i = 0; i < active_memcpy_htd.size(); i++) {						
						if(active_memcpy_htd.get(i).getPinned() != 1) {
							if(Float.compare(kernel.getDuration(),1.0f) >0 && 
									Float.compare(active_memcpy_htd.get(i).getExpected_left_time(),3.0f) < 0 )
								active_memcpy_htd.get(i).setExpected_left_time(0.4f * randQuery.nextFloat() + active_memcpy_htd.get(i).getExpected_left_time());

							if(Float.compare(active_memcpy_htd.get(i).getK().getDuration(), 5.0f) > 0)
...
//											+ 0.01f
					...
					for (int i = 0; i < active_memcpy_dth.size(); i++) {
						if(Float.compare(kernel.getDuration(),1.0f) >0 && 
								Float.compare(active_memcpy_dth.get(i).getExpected_left_time(), 3.0f) < 0)
							active_memcpy_dth.get(i).setExpected_left_time(0.4f * randQuery.nextFloat() + active_memcpy_dth.get(i).getExpected_left_time());
//						else 
//							active_memcpy_dth.get(i).setExpected_left_time(0.1f * randQuery.nextFloat() + active_memcpy_dth.get(i).getExpected_left_time());
						...
						int client_id=active_memcpy_dth.get(i).getK().getQuery_type();
						
						if(current.getK().getQuery_type()!=0 && active_memcpy_dth.get(i).getK().getQuery_type() == 0
								&& issueIndicator.get(client_id) == 1) {
//							System.out.println("end time updated from: "+finishedQueries.get(client_id).getLast().getEnd_time()+" to "+
//								(start_t+ active_memcpy_dth.get(i).getExpected_left_time()) );
							finishedQueries.get(client_id).getLast().setEnd_time(start_t+ active_memcpy_dth.get(i).getExpected_left_time());
						}
					}
					}
...
//					  +", ready: "+issuingQueries.get(kernel.getQuery_type()).getReady_time()
					  + ".----delay by: "+(kernel.getStart_time() - issuingQueries.get(kernel.getQuery_type()).getReady_time())); 
//			  else 
//			  System.out.println(kernel.getQuery_type()+"----"+kernel.getExecution_order()+
...
//							  && Float.compare(issuingQueries.get(k.getQuery_type()).
...
//				kernel.setOccupancy(new Integer(profile[5]).intValue());
	...
	static float bg_st[] = { 0.0f, 33.0f, 33.1f, 92.0f, 92.1f,
			92.2f, 92.3f, 92.4f };
...
//					+detail_conf_name +"~"+finishedQueries.get(i).get(0).getQuery_name()+"-sim.csv");
			...
			try {
				...
				bw = new BufferedWriter(
					...
					bw = new BufferedWriter(
							new FileWriter(MPSSim.RESULT_PATH
									+ detail_conf_name +"~"+finishedQueries.get(i).get(0).getQuery_name()+"-nomps-sim.csv", true));					
				}
//				BufferedWriter bw = new BufferedWriter(
//						new FileWriter(MPSSim.RESULT_PATH
//								+ finishedQueries.get(i).peek().getQuery_name()
//								+ "-" + n_bg + "-" + bg_name + "-tg-sim.csv", true));
				...
				for (Query finishedQuery : finishedQueries.get(i)) {
					...
					if (finishedQueries.get(i).peek().getQuery_name()
						...
						real_latency = finishedQuery.getEnd_time()
								- finishedQuery.getStart_time()
								+finishedQuery.getOverhead() + 5.0f;
					} else if (finishedQueries.get(i).peek().getQuery_name()
							.equals("face")) {
						real_latency = finishedQuery.getEnd_time()
								- finishedQuery.getStart_time()+finishedQuery.getOverhead()+randQuery.nextFloat()*60.0f;						
					} else {
						real_latency = finishedQuery.getEnd_time()
								- finishedQuery.getStart_time()+finishedQuery.getOverhead();
						// real_latency =
						// finishedQuery.getEnd_time()-finishedQuery.getStart_time();
					}
					// */
...
//							/ finishedQueries.get(i).size()) + "(ms)");
		...
		if (true) {			
			...
			for (int i = targetQueries.size(); i < finishedQueries.size(); i++) {
				ArrayList<Float> all_latency = new ArrayList<Float>();
				try {
					BufferedWriter bg_bw;
					
					if(MPS_enabled) {
					bg_bw = new BufferedWriter(
							new FileWriter(MPSSim.RESULT_PATH
									+ detail_conf_name +"~"+finishedQueries.get(i).get(0).getQuery_name()+"-sim.csv", true));
					} else {
						bg_bw = new BufferedWriter(
...
//					 			+", duration: "+(finishedQuery.getEnd_time()-finishedQuery.getStart_time())+", complete: "+COMPLETE_TIME);
	...
	private static float getSlack(String query_name) {
		...
		case "stemmer":
			...
			// Rodinia~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		case "backprop":
			return 1.0f+randQuery.nextFloat();
		case "bfs":
			return randQuery.nextFloat();
		case "cfd":
			return 0.05f;
...
//			return 1.1f + randQuery.nextFloat() * 0.1f;
		...
		case "hybridsort":
			return 1.0f;
		case "kmeans":
			return 1.1f+randQuery.nextFloat()*0.05f;
		case "lavaMD":
			return 1.0f+randQuery.nextFloat()*0.1f;
//			return 1.0f;
		case "leukocyte":
			return 1.0f;
		case "lud":
